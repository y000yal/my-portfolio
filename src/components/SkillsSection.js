...
const SkillsSection = () => {
  ...
  useEffect(() => {
    if (responseData) {
      ...
      if (apiParams.search) {
        ...
        filtered.data = Object.keys(responseData.data).reduce((acc, category) => {
          acc[category] = {};
          Object.keys(responseData.data[category]).forEach(subcategory => {
            acc[category][subcategory] = responseData.data[category][subcategory].filter(skill =>
      ...
      if (apiParams.category !== 'all') {
        filtered.data = Object.keys(responseData.data).reduce((acc, category) => {
          if (category.toLowerCase().includes(apiParams.category.toLowerCase())) {
      ...
      if (apiParams.skill_level !== 'all') {
        filtered.proficiency_levels = {
          [apiParams.skill_level]: responseData.proficiency_levels[apiParams.skill_level] || []
        };
      }
  ...
  const renderPreview = () => {
    ...
    const getSkillLevel = (skillName) => {
      if (filteredData.proficiency_levels?.expert?.includes(skillName)) return "Expert";
      if (filteredData.proficiency_levels?.advanced?.includes(skillName)) return "Advanced";
      if (filteredData.proficiency_levels?.intermediate?.includes(skillName)) return "Intermediate";
      if (filteredData.proficiency_levels?.beginner?.includes(skillName)) return "Beginner";
      return "Intermediate";
    };
            ...
            <div className="skills-grid">
              {Object.keys(filteredData.data[category]).map(subcategory => (
                <div key={subcategory} className="skill-category">
                  ...
                  {filteredData.data[category][subcategory].map((skill, index) => (
    ...
    <div className="api-section" id="skills-section">
      ...
      <div className="url-builder-container">
        ...
        <div className="url-builder">
               ...
               <span className="url-base">/api/skills</span><span className="url-params">
              {(() => {
                const params = [];
                if (apiParams.search) params.push(`search=${encodeURIComponent(apiParams.search)}`);
                if (apiParams.category !== 'all') params.push(`category=${encodeURIComponent(apiParams.category)}`);
                if (apiParams.skill_level !== 'all') params.push(`skill_level=${encodeURIComponent(apiParams.skill_level)}`);
                if (!apiParams.include_levels) params.push('include_levels=false');
                return params.length > 0 ? `?${params.join('&')}` : '';
              })()}
            </span>
          </div>
          <div className="url-copy-btn" onClick={() => navigator.clipboard.writeText(`/api/skills${(() => {
            const params = [];
            if (apiParams.search) params.push(`search=${encodeURIComponent(apiParams.search)}`);
            if (apiParams.category !== 'all') params.push(`category=${encodeURIComponent(apiParams.category)}`);
            if (apiParams.skill_level !== 'all') params.push(`skill_level=${encodeURIComponent(apiParams.skill_level)}`);
        ...
        <p>Retrieve technical skills and expertise data with customizable parameters</p>
      </div>
      
      <div className="api-content">
        <div className="request-panel">
          <div className="panel-header">
            <h3><FiFilter /> Request Parameters</h3>
            <button onClick={resetParams} className="reset-params-btn" title="Reset to defaults">
              <FiRefreshCw />
            </button>
          </div>

          <div className="param-group">
            <h4><FiSearch /> Search & Filter</h4>
            ...
            <div className="param-input-group">
              ...
              <select
                ...
                onChange={(e) => handleParamChange('skill_level', e.target.value)}
                className="param-select"
              >
            ...
            <div className="param-input-group">
              <label>
                <input
                  ...
                  onChange={(e) => handleParamChange('include_levels', e.target.checked)}
